@php
    ...
    $allTemplates = $allTemplates ?? collect($catalog->templates())->filter(fn ($t) => ($t['aktif_katalog'] ?? true))->all();
...
<head>
    ...
    <meta name="description" content="Lihat katalog undangan digital Rayakan Momen: pernikahan, ulang tahun anak, dan untuk pasangan. Pilih desain, lihat demo, pesan via WhatsApp.">
    ...
    <style>
        body.katalog-page { background: #f7f4ef; }
        .katalog-banner {
        ...
        .katalog-banner p { margin: .6rem 0 0; color: rgba(255,255,255,.55); font-size: .9rem; }
        .katalog-toolbar {
            position: sticky;
            top: 4rem;
            z-index: 40;
        ...
        @media (min-width: 768px) { .katalog-toolbar { top: 4.5rem; } }
        .katalog-chips {
            display: flex;
            gap: .5rem;
            overflow-x: auto;
        ...
        .katalog-chips::-webkit-scrollbar { display: none; }
        .katalog-chip {
            flex: 0 0 auto;
            border: 0;
            border-radius: 999px;
        ...
        @media (min-width: 768px) {
            .katalog-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 1.25rem; }
        }
        @media (min-width: 1100px) {
            .katalog-grid { grid-template-columns: repeat(4, minmax(0, 1fr)); }
        }
        .produk-card {
            background: #fff;
            border-radius: 1rem;
        ...
        .produk-card.is-hidden { display: none; }
        .produk-card__media {
            position: relative;
            aspect-ratio: 3/4;
            background: #1a2234;
        ...
        @media (min-width: 768px) { .produk-card__title { font-size: 1.15rem; } }
        .produk-card__mulai {
            font-size: .62rem;
            letter-spacing: .08em;
            text-transform: uppercase;
        ...
        .produk-empty.is-show { display: block; }
        .katalog-back {
            display: inline-flex;
            align-items: center;
            gap: .4rem;
        ...
        .katalog-back:hover { color: #e8d5a3; }
    </style>
</head>
    ...
    <div class="katalog-toolbar">
        <div class="max-w-6xl mx-auto px-4 sm:px-6">
            <div class="katalog-chips" id="katalogFilters" role="tablist">
                <button type="button" class="katalog-chip {{ $activeKat === 'all' ? 'is-active' : '' }}" data-filter="all">Semua</button>
                @foreach ($categories as $kat)
                    <button type="button" class="katalog-chip {{ $activeKat === $kat['id'] ? 'is-active' : '' }}" data-filter="{{ $kat['id'] }}">{{ $kat['nama'] }}</button>
                @endforeach
            </div>
        </div>
    </div>
                ...
                <article class="produk-card {{ $hidden ? 'is-hidden' : '' }}" data-category="{{ $t['kategori'] }}">
                    ...
                    <div class="produk-card__body">
                        ...
                        <div class="produk-card__actions">
                            @if (! empty($t['demo_url']))
                                <a href="{{ $t['demo_url'] }}" target="_blank" rel="noopener noreferrer" class="produk-btn produk-btn--demo">Demo</a>
                            @else
                                <span class="produk-btn produk-btn--demo" style="opacity:.45;cursor:default">Demo</span>
                            @endif
                            <a href="https://wa.me/{{ $waNumber }}?text={{ $waText }}" target="_blank" rel="noopener noreferrer" class="produk-btn produk-btn--pakai">Pakai</a>
                        </div>
                    </div>
                </article>
            @endforeach
