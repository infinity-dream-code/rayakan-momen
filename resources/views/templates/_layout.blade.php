{{--
  ...
  - templates/classic.blade.php
  - templates/langit_malam.blade.php
--}}
@php
    $cover = $theme['cover'] ?? 'https://images.unsplash.com/photo-1519741497674-611481863552?auto=format&fit=crop&w=900&q=80';

    $ytId = null;
    if (!empty($undangan['youtube_url']) && preg_match('/(?:v=|youtu\.be\/|embed\/)([a-zA-Z0-9_-]{11})/', $undangan['youtube_url'], $m)) {
        $ytId = $m[1];
    }
...
<head>
    ...
    <style>
        ...
        .invite-wrap { max-width: 480px; margin: 0 auto; min-height: 100vh; position: relative; }
        .cover-hero {
            min-height: 100vh; display: flex; flex-direction: column; align-items: center;
            justify-content: center; text-align: center; padding: 2rem 1.5rem 6rem; position: relative; overflow: hidden;
        }
        .cover-hero::before {
            content: ''; position: absolute; inset: 0; z-index: 1;
            background: {{ $theme['overlay'] }};
        }
        .cover-bg { position: absolute; inset: 0; background-size: cover; background-position: center; filter: {{ $theme['cover_filter'] }}; }
        .cover-content { position: relative; z-index: 2; }
        .section-block { padding: 3rem 1.5rem; }
        .divider {
            width: 64px; height: 1px; margin: 1rem auto;
        ...
        .nav-bottom {
            ...
            display: flex; gap: 0.25rem; padding: 0.5rem 0.75rem; border-radius: 999px;
            background: {{ $theme['nav_bg'] }}; border: 1px solid var(--border);
            box-shadow: 0 10px 30px rgba(0,0,0,0.15); backdrop-filter: blur(10px);
        }
        .nav-bottom a {
            width: 2.5rem; height: 2.5rem; border-radius: 50%;
            display: flex; align-items: center; justify-content: center;
            color: var(--muted); font-size: 0.875rem; transition: .2s;
        }
        .nav-bottom a:hover { background: var(--accent); color: var(--btn-text); }
        .form-field {
            width: 100%; border-radius: 0.75rem; padding: 0.75rem 1rem;
            border: 1px solid var(--border); background: {{ $theme['input_bg'] }};
        ...
        .timeline-item { position: relative; padding-left: 1.5rem; border-left: 1px solid var(--accent); margin-left: 0.5rem; padding-bottom: 1.5rem; }
...
<div class="invite-wrap">
    <section id="cover" class="cover-hero">
        ...
        <div class="cover-content">
            ...
            @if ($tanggalFormatted)
                <p class="text-sm tracking-wide mt-4" style="color:var(--muted)">{{ strtoupper($tanggalFormatted) }}</p>
            @endif
    ...
    <div id="isi-undangan" class="hidden">
        <section id="mempelai" class="section-block text-center">
            ...
            @if (!empty($undangan['kutipan']))
                <p class="font-display italic text-sm leading-relaxed mb-2 max-w-sm mx-auto" style="color:var(--muted)">
                    "{{ $undangan['kutipan'] }}"
                </p>
            ...
            <div class="space-y-8 mt-6">
                <div>
                    @if (!empty($undangan['foto_wanita']))
                        <img src="{{ asset($undangan['foto_wanita']) }}" alt="{{ $undangan['nama_wanita'] }}" class="w-28 h-28 object-cover rounded-full mx-auto mb-4 border-2" style="border-color: var(--accent);">
                    @endif
                    <h2 class="font-display text-3xl mb-1" style="color:var(--accent)">{{ $undangan['nama_wanita'] }}</h2>
                    ...
                    @if (!empty($undangan['ortu_wanita']))
                        <p class="text-xs mt-2" style="color:var(--muted)">Putri dari<br>{{ $undangan['ortu_wanita'] }}</p>
                    @endif
                </div>
                <p class="font-script text-3xl" style="color:var(--accent)">&</p>
                <div>
                    @if (!empty($undangan['foto_pria']))
                        <img src="{{ asset($undangan['foto_pria']) }}" alt="{{ $undangan['nama_pria'] }}" class="w-28 h-28 object-cover rounded-full mx-auto mb-4 border-2" style="border-color: var(--accent);">
                    @endif
                    <h2 class="font-display text-3xl mb-1" style="color:var(--accent)">{{ $undangan['nama_pria'] }}</h2>
                    @if (!empty($undangan['nama_lengkap_pria']))
                        <p class="text-sm font-medium">{{ $undangan['nama_lengkap_pria'] }}</p>
                    @endif
                    @if (!empty($undangan['ortu_pria']))
                        <p class="text-xs mt-2" style="color:var(--muted)">Putra dari<br>{{ $undangan['ortu_pria'] }}</p>
                    @endif
                </div>
            </div>
        </section>
        ...
        <section id="acara" class="section-block">
            <p class="text-xs tracking-[0.2em] uppercase text-center mb-2" style="color:var(--accent)">Waktu & Tempat</p>
            <h2 class="font-display text-2xl text-center mb-6">Detail Acara</h2>
            <div class="space-y-4">
                @if (!empty($undangan['tempat_akad']) || !empty($undangan['tanggal_akad']))
                    <div class="card-soft text-center">
                        <p class="text-xs tracking-widest uppercase mb-2" style="color:var(--accent)">Akad Nikah</p>
                        @if (!empty($undangan['tanggal_akad']))
                            <p class="font-display text-lg">{{ \Illuminate\Support\Carbon::parse($undangan['tanggal_akad'])->locale('id')->translatedFormat('l, d F Y') }}</p>
                        @endif
                        @if (!empty($undangan['waktu_akad']))
                            <p class="text-sm mt-1" style="color:var(--muted)">{{ $undangan['waktu_akad'] }}</p>
                        @endif
                        @if (!empty($undangan['tempat_akad']))
                            <p class="font-medium text-sm mt-3">{{ $undangan['tempat_akad'] }}</p>
                        @endif
                        @if (!empty($undangan['alamat_akad']))
                            <p class="text-xs mt-1" style="color:var(--muted)">{{ $undangan['alamat_akad'] }}</p>
                        @endif
                    </div>
                @endif
                @if (!empty($undangan['tempat_resepsi']) || !empty($undangan['tanggal_resepsi']))
                    <div class="card-soft text-center">
                        <p class="text-xs tracking-widest uppercase mb-2" style="color:var(--accent)">Resepsi</p>
                        @if (!empty($undangan['tanggal_resepsi']))
                            <p class="font-display text-lg">{{ \Illuminate\Support\Carbon::parse($undangan['tanggal_resepsi'])->locale('id')->translatedFormat('l, d F Y') }}</p>
                        @endif
                        @if (!empty($undangan['waktu_resepsi']))
                            <p class="text-sm mt-1" style="color:var(--muted)">{{ $undangan['waktu_resepsi'] }}</p>
                        @endif
                        @if (!empty($undangan['tempat_resepsi']))
                            <p class="font-medium text-sm mt-3">{{ $undangan['tempat_resepsi'] }}</p>
                        @endif
                        @if (!empty($undangan['alamat_resepsi']))
                            <p class="text-xs mt-1" style="color:var(--muted)">{{ $undangan['alamat_resepsi'] }}</p>
                        @endif
                    </div>
                @endif
                @if (!empty($undangan['maps_url']))
                    <div class="text-center pt-2">
                        <a href="{{ $undangan['maps_url'] }}" target="_blank" rel="noopener" class="btn-accent text-sm">
        ...
        @if (!empty($undangan['cerita']))
            <section id="cerita" class="section-block">
                <p class="text-xs tracking-[0.2em] uppercase text-center mb-2" style="color:var(--accent)">Our Story</p>
                <h2 class="font-display text-2xl text-center mb-8">Perjalanan Kami</h2>
                <div class="max-w-sm mx-auto">
                    @foreach ($undangan['cerita'] as $c)
                        <div class="timeline-item">
                            <p class="text-xs font-semibold" style="color:var(--accent)">{{ $c['tahun'] ?? '' }}</p>
                            <h3 class="font-display text-lg mt-0.5">{{ $c['judul'] ?? '' }}</h3>
                            <p class="text-sm mt-1 leading-relaxed" style="color:var(--muted)">{{ $c['deskripsi'] ?? '' }}</p>
                        </div>
                    @endforeach
                </div>
            </section>
        ...
        @if (!empty($undangan['galeri']))
            <section id="galeri" class="section-block">
                <p class="text-xs tracking-[0.2em] uppercase text-center mb-2" style="color:var(--accent)">Galeri</p>
                <h2 class="font-display text-2xl text-center mb-6">Momen Kami</h2>
                <div class="grid grid-cols-2 gap-2">
                    @foreach ($undangan['galeri'] as $g)
                        <img src="{{ asset($g) }}" alt="" class="w-full aspect-square object-cover rounded-xl" loading="lazy">
                    @endforeach
                </div>
            </section>
        @endif

        @if (!empty($undangan['rekening']) || !empty($undangan['qris_image']) || !empty($undangan['ewallet']))
            <section id="gift" class="section-block">
                <p class="text-xs tracking-[0.2em] uppercase text-center mb-2" style="color:var(--accent)">Wedding Gift</p>
                <h2 class="font-display text-2xl text-center mb-3">Tanda Kasih</h2>
                <p class="text-sm text-center mb-6 max-w-xs mx-auto" style="color:var(--muted)">Doa restu Anda sangat berarti. Jika berkenan, dapat melalui rekening / QRIS / e-wallet berikut.</p>

                @if (!empty($undangan['qris_image']))
                    <div class="card-soft text-center mb-4">
                        <p class="text-xs tracking-widest uppercase mb-3" style="color:var(--accent)">Scan QRIS</p>
                        <img src="{{ asset($undangan['qris_image']) }}" alt="QRIS" class="w-48 h-48 object-contain mx-auto rounded-xl bg-white p-2">
                    </div>
                @endif
                    ...
                    <div class="space-y-3 mb-4">
                        @foreach ($undangan['rekening'] as $r)
                            <div class="card-soft text-center">
                                ...
                                <p class="text-xs mt-1" style="color:var(--muted)">a.n. {{ $r['atas_nama'] ?? '' }}</p>
                            </div>
                        @endforeach
                    </div>
                @endif

                @if (!empty($undangan['ewallet']))
                    <div class="space-y-3">
                        @foreach ($undangan['ewallet'] as $e)
                            <div class="card-soft text-center">
                                <p class="font-semibold text-sm" style="color:var(--accent)">{{ $e['tipe'] ?? '' }}</p>
                                <p class="font-display text-xl mt-1 tracking-wide">{{ $e['nomor'] ?? '' }}</p>
                                <p class="text-xs mt-1" style="color:var(--muted)">a.n. {{ $e['atas_nama'] ?? '' }}</p>
                            </div>
                        @endforeach
                    </div>
                @endif
        ...
        <section id="ucapan" class="section-block pb-28">
            ...
            @if (session('success'))
                <div class="mb-4 text-sm text-center px-4 py-3 rounded-xl" style="background:rgba(16,185,129,0.15);color:#10b981;">
                    {{ session('success') }}
                </div>
            @endif

            <form method="POST" action="{{ route('undangan.ucapan', $undangan['slug']) }}" class="card-soft space-y-3 mb-8">
                @csrf
                <input type="text" name="nama" placeholder="Nama kamu" class="form-field" required value="{{ old('nama') }}">
            ...
            <div class="space-y-3 max-h-80 overflow-y-auto">
                ...
                @empty
                    <p class="text-center text-sm" style="color:var(--muted)">Jadilah yang pertama memberi ucapan.</p>
                @endforelse
            </div>
        </section>
...
<iframe id="yt-player" class="hidden-audio" src="https://www.youtube.com/embed/{{ $ytId }}?enablejsapi=1&loop=1&playlist={{ $ytId }}" allow="autoplay"></iframe>
@endif
