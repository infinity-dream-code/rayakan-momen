...
<head>
    ...
    <style>
        ...
        .sidebar {
            background: linear-gradient(180deg, #0e1320 0%, #1a2234 100%);
        }
...
<body class="min-h-screen">
        ...
        <aside class="sidebar w-64 shrink-0 hidden md:flex flex-col fixed inset-y-0 left-0 z-30">
            ...
            <nav class="flex-1 px-3 py-5 space-y-1">
                <a href="{{ route('admin.dashboard') }}"
                    class="nav-item {{ request()->routeIs('admin.dashboard') ? 'active' : '' }} flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm">
                    <i class="fa-solid fa-chart-line w-5 text-center"></i> Dashboard
                </a>
                <a href="{{ route('admin.undangan.index') }}"
                    class="nav-item {{ request()->routeIs('admin.undangan.*') ? 'active' : '' }} flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm">
                    <i class="fa-solid fa-envelope-open-text w-5 text-center"></i> Undangan
                </a>
                <a href="{{ route('admin.jenis.index') }}"
                    class="nav-item {{ request()->routeIs('admin.jenis.*') ? 'active' : '' }} flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm">
                    <i class="fa-solid fa-layer-group w-5 text-center"></i> Jenis
                </a>
                <a href="{{ route('admin.setting.index') }}"
                    class="nav-item {{ request()->routeIs('admin.setting.*') ? 'active' : '' }} flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm">
                    <i class="fa-solid fa-gear w-5 text-center"></i> Setting
                </a>
                <a href="{{ route('admin.transaksi.index') }}"
                    class="nav-item {{ request()->routeIs('admin.transaksi.*') ? 'active' : '' }} flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm">
                    <i class="fa-solid fa-receipt w-5 text-center"></i> Transaksi
                </a>
                <a href="{{ route('admin.campaign.index') }}"
                    class="nav-item {{ request()->routeIs('admin.campaign.*') ? 'active' : '' }} flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm">
                    <i class="fa-solid fa-bullhorn w-5 text-center"></i> Campaign
                </a>
                <a href="{{ route('landing') }}" target="_blank"
                    class="nav-item flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm">
            ...
            <div class="px-3 py-5 border-t border-white/10">
                <form method="POST" action="{{ route('admin.logout') }}">
                    @csrf
                    <button type="submit"
                        class="nav-item w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm text-left">
            ...
            <header class="bg-white/80 backdrop-blur border-b border-[#e5e0d8] sticky top-0 z-20">
                <div class="px-4 sm:px-6 py-4 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        <span
                            class="hidden sm:inline text-sm text-[#666]">{{ session('demo_admin_name', 'Admin') }}</span>
                        <a href="{{ route('admin.undangan.create') }}"
                            class="btn-gold inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm">
    ...
    <nav class="md:hidden fixed bottom-0 inset-x-0 bg-[#0e1320] border-t border-white/10 flex z-40">
        <a href="{{ route('admin.dashboard') }}"
            class="flex-1 py-3 text-center text-xs {{ request()->routeIs('admin.dashboard') ? 'text-[#e8d5a3]' : 'text-white/50' }}">
            <i class="fa-solid fa-chart-line block mb-1"></i> Dashboard
        </a>
        <a href="{{ route('admin.undangan.index') }}"
            class="flex-1 py-3 text-center text-xs {{ request()->routeIs('admin.undangan.*') ? 'text-[#e8d5a3]' : 'text-white/50' }}">
            <i class="fa-solid fa-envelope-open-text block mb-1"></i> Undangan
        </a>
        <a href="{{ route('admin.setting.index') }}"
            class="flex-1 py-3 text-center text-xs {{ request()->routeIs('admin.setting.*') ? 'text-[#e8d5a3]' : 'text-white/50' }}">
            <i class="fa-solid fa-gear block mb-1"></i> Setting
        </a>
        <a href="{{ route('admin.campaign.index') }}"
            class="flex-1 py-3 text-center text-xs {{ request()->routeIs('admin.campaign.*') ? 'text-[#e8d5a3]' : 'text-white/50' }}">
